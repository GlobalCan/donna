...
- **Phoenix → Jaeger swap (2026-04-24)** — Phoenix 14.x shipped broken upstream (ModuleNotFoundError in the image itself). Swapped the trace backend to `jaegertracing/all-in-one:1.60` — same OTLP-gRPC port (4317), hostname renamed in `docker-compose.yml` + `config.py`. Jaeger UI at `:16686`, port-forward via SSH. **Think inherits the same OTEL exporter; it'll export to Jaeger automatically with zero code changes.** Tradeoff: Jaeger is a generic distributed-tracing UI, not LLM-native (no special prompt/completion view); in-memory storage by default (spans lost on container restart; audit spans also land in the `traces` SQLite table). If Phoenix fixes their image later, swap back is a hostname change.
