...
|---|---|---|---|
| V50-1 | **`not_in_channel` infinite retry storm.** When the bot tries to deliver an outbox row to a channel it isn't a member of, `_post_update` returns False, the row stays, and the drainer retries every ~1.5s **forever**. Hit during smoke when a stale outbox row from a pre-rename `/ask` test couldn't deliver to its originating channel. Operator had to manually `DELETE FROM outbox_updates` to stop the spam. | **FIXED v0.5.1** | `slack_errors.py` classifier (TRANSIENT/TERMINAL/UNKNOWN), `outbox_dead_letter` table (migration 0009), `Retry-After` honoring + per-channel cool-down (capped 5min), throttled operator DM, structured observability log lines. 25 new tests. |
...
| V50-5 | **Slack "Reinstall to Workspace" doesn't always rotate the bot token.** Operator hit this when trying to rotate after accidentally pasting tokens in chat. Real rotation requires "Revoke All OAuth Tokens" → reinstall. | **DOCUMENTED v0.5.1** | Full runbook at `docs/slack/TOKEN_ROTATION.md` covering bot/app/signing/client tokens + secrets sync + verification + audit. |
...
| V50-7 | **Validator footer renders `:warning:` as text in Slack.** The `⚠️ partial validation` badge sometimes shows as `:warning: partial validation` due to Slack's emoji shortcode handling adjacent to formatting. | **FIXED v0.5.1** | Root cause: glyph wrapped INSIDE `_..._` italic markers. Fix: hoist glyph out of italic span (`grounded.py::_format_output`). 2 regression tests. |
| V50-8 | **Dual-field memory deferred.** Codex's recommended next iteration: persist `raw_content` (audit) + `safe_summary` (prompt-side rendering) for tainted assistant rows, instead of storing the raw and rendering with a wrapper as v0.4.4 does. | **FIXED v0.5.2** | Migration 0010 adds `messages.safe_summary`; `JobContext.finalize` captures the assistant row id and the post-finalize hook fires `_backfill_safe_summary` (Haiku via existing `sanitize_untrusted`); `compose_system` splits tainted rows into sanitized (rendered unwrapped as continuity dialogue) and raw-only (rendered with v0.4.4 wrapper as fallback). 11 new tests. |
...
| V60-4 | **slack-doctor over-fails on `users.conversations: missing_scope`.** Donna's manifest deliberately doesn't grant `channels:read` (Codex review: "would let bot read all channel chat, not just mentions. Privacy + token blast radius"). slack-doctor's "list member channels" probe needs that scope and throws missing_scope. Bot is functionally healthy regardless. | **FIXED v0.6.2** | Demoted `missing_scope` on the channel-listing path to a warning ("scope not granted; channel listing skipped") rather than a fail. Other Slack errors on the same path still fail loud. 2 regression tests in `test_botctl_slack_doctor.py`. |
| V60-5 | **Operator could not stop a runaway schedule from Slack.** 2026-05-02 incident: `* * * * *` test schedule fired SCHED_OK every minute into #donna-test for ~30 minutes. Operator tried `/donna_cancel sch_...` twice and it returned "cancelled" both times — but the schedule kept firing. Root cause: pre-fix `/donna_cancel` always called `jobs_mod.set_status` against the `jobs` table, which executes `UPDATE ... WHERE id = ?` and returns True regardless of rowcount when worker_id is None. Schedule IDs missed every row but the slash command claimed success. Compounding: there was no Slack-callable schedule disable at all — only `botctl schedule disable <sid>` via SSH. | **FIXED v0.6.2** | Smart-route `/donna_cancel` by ID prefix: `sch_...` → `disable_schedule`, otherwise → job cancel with existence check. New `/donna_schedule_disable <sid>` slash command for explicit semantics. Added module-level helpers `_route_cancel_or_disable` / `_disable_schedule_by_id` / `_cancel_job_by_id` so the routing is unit-testable without a bolt App. 10 regression tests in `test_slack_ux_cancel_routing.py`. Manifest updated for the new command. |
| V60-6 | **`schedules.target_channel_id` semantically half-wired.** Codex 2026-05-02 review: the column was set by the modal + CLI but `_resolve_channel_for_job` ignored it, reading `threads.channel_id` via `jobs.thread_id`. V50-2 worked only because the modal co-set both fields; operator-edited target_channel_id was a silent no-op. Docstring on the resolver claimed "priority 1: schedule.target_channel_id" while implementation did no such thing. | **FIXED v0.6.3** | Migration 0012 adds `jobs.schedule_id` back-link. `Scheduler._fire` propagates the link. `_resolve_channel_for_job` now honestly checks: (1) schedule.target_channel_id when job.schedule_id set, (2) thread.channel_id otherwise. 8 integration tests in `test_target_channel_id_resolver.py`. Closes the half-wired risk before morning brief depends on it. |
